...
DemonTank::DemonTank(int startX, int startY, SDL_Renderer *renderer, int health) {
    moveDelay = 5;
    x = startX;
    y = startY;
    hp = health;
...
void DemonTank::moveTowardPlayer(int playerX, int playerY, const vector<Wall>& walls, const vector<Stone>& stones) {
    if (--moveDelay > 0) return;
...
bool DemonTank::canMove(int newX, int newY, const vector<Wall>& walls, const vector<Stone>& stones) {
    if (newX < TILE_SIZE || newX > SCREEN_WIDTH - TILE_SIZE * 5 - TILE_SIZE * 2 ||
        newY < TILE_SIZE || newY > SCREEN_HEIGHT - TILE_SIZE * 2) {
        return false;
    }
...
void DemonTank::applyMove(int dx, int dy) {
    x += dx;
    y += dy;
    rect.x = x;
    rect.y = y;
...
void DemonTank::shoot(SDL_Renderer* renderer) {
    Uint32 currentTime = SDL_GetTicks();
    if (currentTime - lastShootTime >= shootCooldown) {
        bullets.push_back(Bullet(
            x + TILE_SIZE / 2 - 5,
...
void DemonTank::render(SDL_Renderer* renderer) {
    if (active) {
        SDL_RenderCopyEx(renderer, demonTexture, NULL, &rect, angle, nullptr, SDL_FLIP_NONE);
        for (auto& bullet : bullets) bullet.render(renderer);
    }
}
